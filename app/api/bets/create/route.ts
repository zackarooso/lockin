...
import { findOrCreateUser, createBet, addParticipant, initDb } from '@/lib/db'
export async function POST(req: NextRequest) {
  try {
    ...
    const bet = await createBet({ creator_user_id: session.userId, subject_user_id, about_self: !!about_self, creator_side: actualSide, text: text.trim(), stake_amount: parseFloat(stake_amount), join_deadline, end_time, proof_photo: !!proof_photo, proof_video: !!proof_video, proof_geolocation: !!proof_geolocation })
