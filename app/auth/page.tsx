...
function AuthInner() {
  ...
  const bgStyle = { minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px' }
  const titleStyle = { fontFamily: 'Bebas Neue, sans-serif', fontSize: 80, letterSpacing: 6, background: 'linear-gradient(135deg, #FF0055, #FF1F6B, #FF6FA0, #00FFE0)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 0.9, marginBottom: 8 }

  return (
    <div style={bgStyle}>
      <div style={{ fontSize: 64, marginBottom: 8 }}>🦩</div>
      <div style={titleStyle}>LOCK IN</div>
      ...
      <div style={{ width: '100%', maxWidth: 320 }}>
        ...
        {step === 'name' && (
          <>
            ...
            <button className="btn-ghost" onClick={() => { setStep('phone'); setError('') }} style={{ marginTop: 10 }}>
              Wrong number?
            </button>
          </>
        )}
  ...
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 40, color: 'var(--pink)' }}>LOCK IN</div>
      </div>
    }>
      <AuthInner />
    </Suspense>
