...
export default function BetDetailPage() {
  ...
  async function handleJoin() {
    if (!joinSide) { showToast('Pick a side first ð¤·'); return }
    if (!joinStake || parseFloat(joinStake) <= 0) { showToast('Enter your stake ð¸'); return }
    setLoading(true)
    ...
    setLoading(false)
    if (res.ok) { showToast("You're in! ð¯"); load() }
    ...
    <div className="screen-pad" style={{ paddingBottom: 60 }}>
      ...
      <div style={{
        ...
        <div style={{ paddingLeft: 12 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
            {bet.about_self ? 'ð Self bet' : `ð About ${bet.subject?.display_name || 'someone'}`}
          </div>
          <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4, marginBottom: 16 }}>
            {bet.text}
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Chip icon="ð¸" label={`$${bet.stake_amount} stake`} />
            {bet.proof_photo && <Chip icon="ð·" label="Photo" />}
            {bet.proof_video && <Chip icon="ð¹" label="Video" />}
            {bet.proof_geolocation && <Chip icon="ð" label="Location" />}
          </div>
        </div>
      </div>
      ...
      {bet.participants?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">ð° Who's in</div>
      ...
      {proofs?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">ð Proof submitted</div>
          {proofs.map((p: any) => (
            <div key={p.id} style={{ marginBottom: 10 }}>
              ...
              {p.latitude && (
                <div style={{ background: 'var(--surface-2)', borderRadius: 12, padding: '12px 16px', fontSize: 13, color: 'var(--text)' }}>
                  ð {p.latitude.toFixed(4)}, {p.longitude.toFixed(4)}
      ...
      {votes?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">ð³ï¸ Votes ({votes.length})</div>
      ...
      {canJoin && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            <SideJoinBtn label="â YES" active={joinSide === 'yes'} color="teal" onClick={() => setJoinSide('yes')} />
            <SideJoinBtn label="â NO" active={joinSide === 'no'} color="pink" onClick={() => setJoinSide('no')} />
          </div>
      ...
      {bet.status === 'settled' && (
        <div style={{
          ...
          <div style={{ fontSize: 36, marginBottom: 8 }}>{bet.winning_side === 'yes' ? 'â' : 'â'}</div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Settle up with your friends IRL ð¸</div>
        </div>
      )}
...
function StatusBadge({ status, winningSide }: { status: string; winningSide?: string }) {
  const map: any = {
    ...
    active:    { label: 'ð¥ Active', bg: 'rgba(255,31,107,0.12)', color: 'var(--pink)' },
    voting:    { label: 'ð³ï¸ Voting', bg: 'rgba(255,215,0,0.12)',  color: 'var(--gold)' },
    settled:   { label: `â ${winningSide?.toUpperCase()} WON`, bg: 'rgba(0,255,224,0.08)', color: 'var(--teal)' },
    nullified: { label: 'â Nullified', bg: 'var(--surface-2)', color: 'var(--text-muted)' },
  }
  ...
  return (
    <div style={{ flex: 1, background: 'var(--surface-2)', borderRadius: 10, padding: '10px 12px' }}>
      ...
      <div style={{ fontSize: 12, color: passed ? 'var(--pink)' : 'var(--text)', fontWeight: 500 }}>
        {new Date(date).toLocaleDateString()} {passed && 'â'}
      </div>
    </div>
  )
}
...
function Loader() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '70dvh', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontSize: 40, animation: 'flamingo-bob 1.5s ease-in-out infinite' }}>ð¦©</div>
    </div>
  )
}
