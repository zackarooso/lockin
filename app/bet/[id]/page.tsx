...
export default function BetDetailPage() {
  ...
  async function handleJoin() {
    if (!joinSide) { showToast('Pick a side first Ã°ÂÂ¤Â·'); return }
    if (!joinStake || parseFloat(joinStake) <= 0) { showToast('Enter your stake Ã°ÂÂÂ¸'); return }
    setLoading(true)
    ...
    setLoading(false)
    if (res.ok) { showToast("You're in! Ã°ÂÂÂ¯"); load() }
    ...
    <div className="screen-pad" style={{ paddingBottom: 60 }}>
      ...
      <div style={{
        ...
        <div style={{ paddingLeft: 12 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>
            {bet.about_self ? 'Ã°ÂÂÂ Self bet' : `Ã°ÂÂÂ About ${bet.subject?.display_name || 'someone'}`}
          </div>
          <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text)', lineHeight: 1.4, marginBottom: 16 }}>
            {bet.text}
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Chip icon="Ã°ÂÂÂ¸" label={`$${bet.stake_amount} stake`} />
            {bet.proof_photo && <Chip icon="Ã°ÂÂÂ·" label="Photo" />}
            {bet.proof_video && <Chip icon="Ã°ÂÂÂ¹" label="Video" />}
            {bet.proof_geolocation && <Chip icon="Ã°ÂÂÂ" label="Location" />}
          </div>
        </div>
      </div>
      ...
      {bet.participants?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">Ã°ÂÂÂ° Who's in</div>
      ...
      {proofs?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">Ã°ÂÂÂ Proof submitted</div>
          {proofs.map((p: any) => (
            <div key={p.id} style={{ marginBottom: 10 }}>
              ...
              {p.latitude && (
                <div style={{ background: 'var(--surface-2)', borderRadius: 12, padding: '12px 16px', fontSize: 13, color: 'var(--text)' }}>
                  Ã°ÂÂÂ {p.latitude.toFixed(4)}, {p.longitude.toFixed(4)}
      ...
      {votes?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">Ã°ÂÂÂ³Ã¯Â¸Â Votes ({votes.length})</div>
      ...
      {canJoin && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
            <SideJoinBtn label="Ã¢ÂÂ YES" active={joinSide === 'yes'} color="teal" onClick={() => setJoinSide('yes')} />
            <SideJoinBtn label="Ã¢ÂÂ NO" active={joinSide === 'no'} color="pink" onClick={() => setJoinSide('no')} />
          </div>
          ...
          <button className="btn-primary" onClick={handleJoin} disabled={loading}>
            {loading ? 'Joining...' : "I'M IN Ã°ÂÂÂ¯"}
          </button>
        </div>
      )}
      ...
      {bet.status === 'settled' && (
        <div style={{
          ...
          <div style={{ fontSize: 36, marginBottom: 8 }}>{bet.winning_side === 'yes' ? 'Ã¢ÂÂ' : 'Ã¢ÂÂ'}</div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Settle up with your friends IRL Ã°ÂÂÂ¸</div>
        </div>
      )}
      ...
      {bet.status === 'nullified' && (
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, textAlign: 'center' }}>
          <div style={{ fontSize: 36, marginBottom: 8 }}>Ã°ÂÂ¤Â·</div>
...
function StatusBadge({ status, winningSide }: { status: string; winningSide?: string }) {
  const map: any = {
    ...
    active:    { label: 'Ã°ÂÂÂ¥ Active', bg: 'rgba(255,31,107,0.12)', color: 'var(--pink)' },
    voting:    { label: 'Ã°ÂÂÂ³Ã¯Â¸Â Voting', bg: 'rgba(255,215,0,0.12)',  color: 'var(--gold)' },
    settled:   { label: `Ã¢ÂÂ ${winningSide?.toUpperCase()} WON`, bg: 'rgba(0,255,224,0.08)', color: 'var(--teal)' },
    nullified: { label: 'Ã¢ÂÂ Nullified', bg: 'var(--surface-2)', color: 'var(--text-muted)' },
  }
  ...
  return (
    <div style={{ flex: 1, background: 'var(--surface-2)', borderRadius: 10, padding: '10px 12px' }}>
      ...
      <div style={{ fontSize: 12, color: passed ? 'var(--pink)' : 'var(--text)', fontWeight: 500 }}>
        {new Date(date).toLocaleDateString()} {passed && 'Ã¢ÂÂ'}
      </div>
    </div>
  )
}
...
function Loader() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '70dvh', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontSize: 40, animation: 'flamingo-bob 1.5s ease-in-out infinite' }}>Ã°ÂÂ¦Â©</div>
    </div>
  )
}
