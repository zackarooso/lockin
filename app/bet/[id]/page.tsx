...
export default function BetDetailPage() {
  ...
  return (
    <div className="screen-pad" style={{ paddingBottom: 60 }}>
      ...
      {votes?.length > 0 && (
        <div style={{ marginBottom: 20 }}>
          <div className="section-label">🗳️ Votes ({votes.length})</div>
...
function StatusBadge({ status, winningSide }: { status: string; winningSide?: string }) {
  const map: any = {
    ...
    voting:    { label: '🗳️ Voting', bg: 'rgba(255,215,0,0.12)',  color: 'var(--gold)' },
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, padding: '4px 12px', borderRadius: 99, background: s.bg, color: s.color, fontFamily: 'Bebas Neue, sans-serif', fontSize: 13 }}>
      {s.label}
    </span>
  )
}
