...
export default function CreateBet() {
  ...
  function stakeLabel(amt: string) {
  ...
  async function submit() {
    ...
    if (new Date(joinDeadline) > new Date(endTime)) { setError('Join-by must be before end time'); return }
      ...
      <div style={{ padding: '16px' }}>
        ...
        {!aboutSelf && (
          <div style={{ marginBottom: 16 }}>
            ...
            <input className="form-input" type="text" value={friendName} placeholder="Their name" onChange={e => setFriendName(e.target.value)} style={{ marginBottom: 10 }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
          <div>
            ...
            <input className="form-input" type="datetime-local" value={joinDeadline} onChange={e => setJoinDeadline(e.target.value)} />
          </div>
          <div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {([['proofPhoto', 'PHOTO', proofPhoto, setProofPhoto], ['proofVideo', 'VIDEO', proofVideo, setProofVideo], ['proofGeo', 'LOCATION', proofGeo, setProofGeo]] as Array<[string, string, boolean, (b: boolean) => void]>).map(([key, label, val, setter]) => (
